...
(() => {
  ...
  function recurringChargeFromText(text) {
    return firstMatch(text, [
      /(?:after (?:the )?trial|then|renews?)[^\d$\u20ac\u00a3\u20b9]{0,40}((?:\$|\u20ac|\u00a3|\u20b9)\s*\d+(?:[.,]\d{1,2})?)/i,
  ...
  function analyzeTrialPage() {
    ...
    const recurringCharge = recurringChargeFromText(text);
    ...
    return {
      ...
      currency: (fee || recurringCharge)?.match(/[\$\u20ac\u00a3\u20b9]/)?.[0] || null,
      evidence,
  ...
  function currencyCode(symbol) {
    return { "$": "USD", "\u20ac": "EUR", "\u00a3": "GBP", "\u20b9": "INR" }[symbol] || "USD";
  }

  function buildProtectionPayload(riskScore) {
    const trial = analyzeTrialPage();
    const text = normalizedPageText();
    const renewalAmount = recurringChargeFromText(text);
    const frequency = text.match(/(?:\/|per\s+)(month|year|week)/i)?.[1]?.toLowerCase() || "unknown";

    // This object is the exact Pydantic ProtectTrialRequest payload sent to
    // FastAPI by serviceworker.js. No form values or raw page HTML are sent.
    return {
      provider_name: trial.provider_name,
      source_url: trial.source_url,
      trial_duration: trial.trial_duration,
      renewal_amount: renewalAmount,
      currency: currencyCode(trial.currency),
